...
func (this *Build_Html) build_pcindex(data map[string]string) {
	...
	contpl = strings.Replace(contpl, "{{model_name}}", data["con_name"], -1)
	url_add := "/admin/page_list/"
...
func (this *Build_Html) build_pcseach_form(content string) string {
	...
	search_js := `$.ajax({
                url: tpl_url_index,
                type: 'POST',
                data: {access_token:access_token{{model_name}}},
                dataType: 'json',
	...
	for _, v := range data {
		if v["f_type"] == "下拉框" {
			...
			if v["list_tb_name"] == "0" {
				...
				/*获取数据库中对应的条件，生成下拉框*/
				db.Dbinit()
				model_name := strings.Replace(v["list_tb_name"], "sl_", ",", -1)
				search_js = strings.Replace(search_js, "{{mode_name}}", model_name, -1)
				list_val := strings.Split(v["list_val"], "|")
				list_display := strings.Split(v["list_display"], "|")
